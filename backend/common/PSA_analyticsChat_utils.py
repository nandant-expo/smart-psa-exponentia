...
def specific_flow_process(id,user_id,workspace):#add userid
    ...
    if status:
        db = Database('AnalyticsCollection_PSA',workspace)
...
def generate_stream_response(query,user_id,chat_id,workspace):
    try:
        ...
        if len(query.strip())>0:
            ...
            # print(db_database.run("select * from doc_collection"))
...
def upload_csv(user_id,df):
    ...
    main_dir = f"SDA_PSA/{user_id}/csv_data"
    storage_uri = client.get_secret("STORAGE-ACCOUNT").value
...
def query_to_dataframe(host,token,query_result,catalog,schema,cluster_id):
    start=time.time()
    # cluster_id='0111-094528-b2hpit56'
...
def natural_language_to_query(data,user_query,error_message):
    ...
    # print(system_message_conversation)
...
def create_new_chat(workspace_name,user_id,user_query,sql_query,chat_id,location):
    chat_meta = {}
    db_analytics = Database('AnalyticsCollection_PSA',workspace_name)
    now = datetime.now().strftime('%d-%m-%Y %H:%M:%S')
    today_date=datetime.now().strftime('%Y-%m-%d')
    if chat_id == None: 
...
def identify_visualization(df,user_query,sql_query):
    ...
    3. Return only the graph Type as a response. Do not give 'Table' as a graph type.
    4. Return graph type which can be possible to create using plotly. 
    4. <b>If there is only one row/column in the data then return "Sorry" in the response.</b>
    5. If a graph cannot be generated, return "Sorry" in the response.
    6. DO NOT PROVIDE ANY OTHER EXTRA EXPLAINATION. ONLY PROVIDE 'GRAPH TYPE' OR 'Sorry' IN THE RESPONSE AS PER ABLOVE INSTRUCTIONS.

    User Query: {user_query}
    Dataframe: 
    {df}
    SQL query from which CSV Data is generated: {sql_query}
    """
...
def check_graph_possibility(user_query,workspace_name,user_id,chat_id):
    ...
    status_central,info=db_central.fetch_one_record({"workspace_name":workspace_name},{"cluster_name":1,"host":1,"token":1})
    # print(info)
    db=Database('tebleMetadata_PSA',workspace_name)
    status_db,data=db.fetch_all_records({"schema_name":"procurement_sda","catalog_name":"dataanalytics"},{"_id":0})
    query_token={}
    ...
    while attempts < max_retries:
        generated_queries=[]
        txt2sql = {}
        query,txt2sql=natural_language_to_query(data,mod_user_query,error_message)
        
        generated_queries.append(query)
        print(query)
        try:
            df=query_to_dataframe(info['host'],info['token'],query.replace('```sql','').replace('```',''),"dataanalytics","procurement_sda",info['cluster_name'])
    ...
    # ddf=identify_groupby(df,query,user_query)
    ...
    visual = {}
    content,visual=identify_visualization(df,mod_user_query,query)
    if "Sorry" in content:
        graph_type=2
...
def save_data(user_id,data,file_type):
    ...
    if file_type=='image':
        ...
        main_dir = f"SDA_PSA/{user_id}/plot_image"
        blob_list = list(container_client.list_blobs(name_starts_with=main_dir))
        file_count = len(blob_list)+1
        file_name=f'graph{file_count}.png'
        file_path=main_dir+'/'+file_name
    if file_type =='json':
        main_dir = f"SDA_PSA/{user_id}/plot_json"
        blob_list = list(container_client.list_blobs(name_starts_with=main_dir))
        file_count = len(blob_list)+1
...
def create_visualisation(user_id,workspace_name,chat_id,file_location,graph_type):
    ...
    if graph_type:
        ...
        df=df.dropna()
        df=df.head(20)
        # pipe_separated_string = df.to_dict(orient='list')
        ...
        system_message_conversation = f"""You are a highly skilled data visualization assistant. Your task is to generate Python code for creating a {graph_type} using the Plotly library based on the provided data. Follow these instructions:
        1. Create a {graph_type} using the Plotly library.
        2. DO not create '# Create a sample dataframe' as dataframe is already defined as df. just use columns from given data in order to create visualisation. ALL THE DATA SHOULD BE COMING FROM BELOW GIVEN DATAFRAME STRICTLY\ndataframe:\n {df}\n.
        ...
        4. First Sort variable df based X-axis column into ascending order then Cast x-axis column into string.
        5. Only and only provide Python code without instructions and do not add Python in the start and in the Python code.
        6. Do not show the figure. The figure variable should always be 'fig'.
        7. DO NOT ADD ANY EXTRA INFORMATION OTHER THAN PYTHON CODE
        """
        response = openai.chat.completions.create(
            model=AZURE_OPENAI_CHATGPT_DEPLOYMENT,
            messages=[{'role': 'user', 'content': system_message_conversation}],
        ...
        # img_bytes = globals()['fig'].to_image(format='png')
        # url=save_data(user_id,img_bytes,'image')

        chat_meta={}
        db_analytics = Database('AnalyticsCollection_PSA',workspace_name)
        analytics_status,analytics_record = db_analytics.fetch_one_record({'_id':chat_id})
        chat_meta.update(analytics_record)
        # chat_meta['chat_conversations'][-1]['image_url']=url
...
def analyse_data(user_id,workspace_name,chat_id,file_location,graph_type,user_query,sql_query):
    ...
    system_message_conversation = f"""Role: You are an expert data analyst.
        Objective: Provide a concise, accurate response based on the user's query. Ensure your answer adheres to the following instructions:
        
        The data provided is the result of the SQL query derived from the user's query. Answer the question strictly based on this data.

        1. Focus on delivering a **precise** and **relevant** answer that **directly** addresses the user’s question, avoiding unnecessary details. Answer language should be same as user's query.
        2. **Do not omit any rows or columns** from the provided data in your response. Include all information as presented in the data.
        3. If the user does not request a table format in the user query, provide a **concise, textual summary** that highlights the key insights from the data, answering the user's question as clearly as possible.
        4. If the user requests a table in the user query then and then only, provide the data in a pipe-separated table format enclosed between <table> and </table> tags. and instead of giving an empty row "|------|-------|" after the header enclose table headers between <thead>| header1 | header2 |</thead> (determine the headers) and table body in <tbody>| cell1 | cell2 |\n| cell3 | cell4 |</tbody> donot use any other tags (like <tr> or <td>) aside from these.
        5. If the dataset contains only one column labeled 'no data available', respond only with: **"Data not available for this query."** Provide no further explanation.
        6. If the dataset is empty but has columns other than 'no data available', and only have one row with column names, without data respond with: **"Data not available for this query."** Do not provide any further clarification.
        8. If the user requests insights that would typically require a graph or visualization, respond with **textual analysis** of the data, focusing on the insights without mentioning the absence of a visual.
        9. **Prioritize efficiency and accuracy**, ensuring the response addresses the user’s specific query without adding unnecessary steps or explanations.
        
        Data: {dict_result}
        User Query: {user_query}
        SQL Query derived from user query: {sql_query}
    """
